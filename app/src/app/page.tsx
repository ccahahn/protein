...
type Step = "receipt" | "loading" | "readout";
...
export default function Home() {
  ...
  const startReadout = async (items: NutritionItem[], storeName: string) => {
    setStore(storeName);
    ...
    try {
      const res = await fetch("/api/readout", {
        ...
        body: JSON.stringify({ store: storeName, receiptItems: items }),
      });
  ...
  const progressCurrent = step === "receipt" ? 1 : 2;
    ...
    <>
      ...
      {step === "receipt" && (
        <>
          ...
          <ReceiptStep
            onComplete={({ store, items }) => startReadout(items, store)}
          />
        </>
      )}
